...
name: pytest
description: >
  Pytest runner and auxiliary patterns for Python.
  Trigger: Only when using pytest as a secondary runner or coverage tool for existing unittest suites.
license: Apache-2.0
metadata:
  author: local
  version: "1.0"
...
In this repository, `unittest` is the canonical test framework.
Do not use this skill as the primary pattern for new tests.
Use it only when pytest is acting as a runner, filter, or coverage wrapper around unittest-based suites.
...
- **Documentation**: [references/project-testing.md](references/project-testing.md)
